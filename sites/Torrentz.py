...
class Torrentz:
    ...
    row = compile(
        "<dl><dt><a href=(?P<path>/[a-z0-9]+)>(?P<name>.*?)</a>",
        MULTILINE | DOTALL | UNICODE,
    )
    ...
    def torrent(self, r):
        ...
        for l in links:
            ...
            if l.startswith("http://www.newtorrents.info"):
                ...
                patt = compile(
                    "<a href='(/down.php\?id=\d+)'><b>download this torrent!</b></a>"
                )
            ...
            if l.startswith("https://kickass.so"):
                ...
                patt = compile(
                    'href="(http://torcache.[^/]+/torrent/[^"]+)"><span>Download torrent</span>'
                )
                torrent = patt.search(otherpage)
            ...
            if l.startswith("https://www.monova.org"):
                ...
                otherpage = cache.get(l, max_age=-1).read()
                patt = compile(
                    '<a id="download-file" href="((?:https:)?//www.monova.org/torrent/download[^"]+)"'
                )
                torrent = patt.search(otherpage)
                if torrent == None:
