...
class LimeTorrents:
    ...
    row = compile(
        '<a href="(?P<path>http://itorrents.org/torrent/[0-9A-Z]+.torrent\?title=[^"]+)" rel="nofollow" class="csprite_dl14".*></a><a href="[^"]+"><(?P<name>[^<]+)/a>'
    )
    ...
    def rows(self, terms, numbers):
        ...
        url = "http://0torrents.com/search/all/%s/" % (
            (terms + numbers).replace(" ", "-")
        )
        print(url)
