...
class TorrentDay:
    ...
    row = compile(
        '<td class=torrentNameInfo><a class="b? hv" href="\/t\/\d+">(?P<name>[^<]+)</a>.*?<a href="(?P<download>/download.php/\d+/[^"]+)">.*?<td class="ac seedersInfo">(?P<seeds>[\d,]+)</td><td class="ac leechersInfo">(?P<peers>[\d,]+)</td>',
        MULTILINE | DOTALL,
    )
    ...
    def rows(self, terms, numbers):
        ...
        return [
            x
            for x in rows
            if x.groupdict()["name"].find("Leaked") == -1
            and x.groupdict()["name"].find("SUBFRENCH") == -1
        ]

    def torrent(self, row):
        return [
            {
                "url": "https://torrentday.it" + html.unescape(row["download"]),
