...
class NyaaTorrents:
    ...
    row = compile(
        """<td class="tlistname"><a href="(?:http:)?//[^/]+/\?page=view&#38;tid=\d+">(?P<name>[^<]+)</a></td>\S*<td class="tlistdownload">.*?<a href="(?P<path>(?:http:)?//[^/]+/\?page=download&#38;tid=\d+)" title="Download"[^>]*><img src="[^\"]+" alt="DL"></a>.*?</td>\S*<td class="tlistsize">\d+.\d+ (?:G|M)iB</td>(?P<items>.+?)</tr>""",
        IGNORECASE | DOTALL,
    )
    item = compile('<td class="([^"]+)"[^>]*>([^<]+)</td>')
    singleitem = compile('<span class="([^"]+)">([^<]+)</span>')
    downloadlink = compile(
        '<div class="viewdownloadbutton"><a href="(//[^/]+/\?page=download&#38;tid=\d+)'
    )
    ...
    def rows(self, terms, numbers):
        terms = " ".join(
            [
                x
                for x in (terms + " " + numbers).split(" ")
                if len(x) > 0 and x[0] != "-"
            ]
        )
        url = "http://www.nyaatorrents.org/?page=search&term=%s&sort=1" % (
            terms.replace(" ", "+")
        )
        print(url)
            ...
            rows.append(
                {
                    "seeds": items["viewsn"],
                    "peers": items["viewln"],
                    "name": otheritems["viewtorrentname"],
                    "path": "http:" + link[0],
                }
            )
        else:
