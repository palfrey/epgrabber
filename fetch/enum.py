...
class EnumInstance:
    ...
    def __repr__(self):
        return "EnumInstance(%s, %s, %s)" % (
            repr(self.__classname),
...
def check_enum(option, opt, value):
    try:
        ...
        raise OptionValueError(
            "'%s' is an invalid type for %s (valid types are %s)"
            % (value, option, ", ".join(option.enum.__members__))
        )
    ...
    def __init__(self, *args, **kwargs):
        if "option_class" in kwargs:
            if not issubclass(kwargs["option_class"], self._option):
                raise Exception(
                    "option_class must be a subclass of %s" % self._option.__name__
                )
        else:
