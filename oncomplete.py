...
parser.add_option(
    "-i",
    "--ignore-torrent",
    help="Do action irregardless of torrent existing",
    action="store_false",
    default=True,
    dest="check_torrent",
)
...
for f in files:
    ...
    if found:
        ...
        for f in goodfiles:
            ...
            if (
                "ID_DEMUXER" in values
                and values["ID_DEMUXER"] == "asf"
                and float(values["ID_VIDEO_FPS"]) == 1000.0
            ) or "ID_AUDIO_ID" not in values:
                nextTorrent = True
                print(values)
                ...
                if not opts.execute:
                    ...
                    if got > 0:
                        if sep in f[len(opts.check_dir) :] and not samefile(
                            dirname(f), opts.check_dir
                        ):
